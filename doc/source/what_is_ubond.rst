...
ubond is a piece of software, similar to OpenVPN_, which can create a network
tunnel between two computers.

ubond encapsulates network packets, using UDP and send them encrypted over the
internet to another location.

The primary use of ubond is to create bonded/aggregated_ network links in order to
benefit from the bandwidth of multiple links.

Still, ubond can be used as a regular secure tunnel daemon, capable of handling failover
scenarios.
...
======================================================
ubond can aggregate very different links if reordering is enabled.
