...
Security in ubond
=================
Security is a very strong focus for the ubond project.

ubond tries it's best to protect your systems and your datas by using `privilege separation`_,
...
====================
ubond needs to access the kernel in order to create a tunnel device, and in order
to configure the device. (needs root)

In order to limit the scope of privileges, ubond spawns a small process, called the
...
The priv process only performs the tasks where it needs privileges:
  ...
  * open the ubond configuration file
  * name resolution (requires to be out of the chroot)

The other process is run as an unprivileged user (usually _ubond or ubond),
...
Cryptography
============
ubond uses `libsodium <http://doc.libsodium.org>`_ for all the cryptographic needs.

In particular, ubond uses `secret-key authenticated encryption <http://doc.libsodium.org/secret-key_cryptography/authenticated_encryption.html>`_.
...
Authentication
--------------
ubond just uses a very simple protocol in order to make sure it communicates only
if the two sides share the same secret key.

The secret key is stored as plain text in the ubond configuration file.

This configuration file must be owned by root, and chmoded to 0600 to prevent
any other user from reading / writing to it.

Every control packet sent by ubond is encrypted and authenticated by both sides.
...
another time using ubond. (encrypting an ssh or https connections does not give you much benefit)
