...
ubond_tuntap_read(struct tuntap_s *tuntap)
{
    ...
    return ubond_tuntap_generic_read(data, ret);
}

int
ubond_tuntap_write(struct tuntap_s *tuntap)
{
    ssize_t ret;
    int datalen = 0;
    ubond_pkt_t *pkt;
    ...
    if (ubond_cb_is_empty(buf))
        fatalx("tuntap_write called with empty buffer");

    pkt = ubond_pktbuffer_read(buf);
...
ubond_tuntap_alloc(struct tuntap_s *tuntap)
{
    ...
    for (i=0; i < 32; i++)
    {
        snprintf(devname, 5, "%s%d",
                 tuntap->type == UBOND_TUNTAPMODE_TAP ? "tap" : "tun", i);
    ...
    if (fd <= 0)
    {
        log_warnx("tuntap",
            ...
            tuntap->type == UBOND_TUNTAPMODE_TAP ? "tap" : "tun");
        return fd;
    }
    tuntap->fd = fd;
